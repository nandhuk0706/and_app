	...
	if($_SERVER["REQUEST_METHOD"])
	{
		...
		switch ($method_name) 
		{
		  ...
		  case 'POST':
			 ...
			 $admin_name=$_REQUEST['cust_id'];
			//$admin_pass=$_REQUEST['admin_pass'];
			//$qty=$_REQUEST['qty'];
			//$qry="SELECT * from product";
			
			
	    	$check_user="select * from example WHERE cust_id='$admin_name";
	    	$run=mysqli_query($dbcon,$check_user);

            if(mysqli_num_rows($run))
            {
                $qry = "select * from admin WHERE cust_id='$admin_name";
			//	$data=array("status"=>"1","message"=>"success","result"=>"Product add successfully");
				$result=mysqli_query($dbcon, $qry);
				while ($row=mysqli_fetch_row($result))
			    {
				$temp_cat = array("admin_id"=>$row[0],"name"=>$row[1],"status"=>"1","message"=>"1");
			    }
                 $data=$temp_cat;
